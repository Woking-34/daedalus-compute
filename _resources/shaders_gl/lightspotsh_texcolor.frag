...
void main(void)
{
	...
	if( diffFactor > 0.0 )
	{
		...
		vec3 lightDir = normalize(lightDirW.xyz);
		float spotFactor = dot(-toLight, lightDir);
		
		float innerAngle = cos(attenLight.z);
		float outerAngle = cos(attenLight.w);   
		
		spotFactor = clamp( (spotFactor - outerAngle) / (innerAngle - outerAngle), 0.0, 1.0 );
		litColor *= spotFactor;
	}
	
	vec3 shadowCoord = v_posL.xyz / v_posL.w;
	shadowCoord = 0.5 * shadowCoord + 0.5;
	float distanceFromLight = texture2D(shadowMap, shadowCoord.xy).r + 0.0001;
	
	float shadowFactor = distanceFromLight < shadowCoord.z ? 0.0 : 1.0;
